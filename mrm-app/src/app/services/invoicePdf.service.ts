...
const NOTA_COMMENT = `Reconhecemos a exatidão desta fatura de locação na importância acima, que pagaremos à "GMA - COMERCIO DE MAQUINAS LTDA" ou a sua ordem na praça e vencimento Indicados.`;
...
export class InvoicePdfService {
  ...
  getContractFromStockItem(itemRental: ItemRental) {
    return [
      {
        text: itemRental.stockItem.name + this.getProductPropertyInfo(itemRental.stockItem),
  ...
  getProductPropertyInfo(stockItem : StockItem) {
    let title = "\nEquipamento de propriedade da GMA";
    let invoiceNumber = '';
    let aquisitionDate = '';

    if (stockItem.serialNumber) {
      invoiceNumber = ", conforme Nota Fiscal n.º " + stockItem.serialNumber;
    }

    if (stockItem.acquisitionDate) {
      aquisitionDate = " Data " + this.datePipe.transform(stockItem.acquisitionDate, "dd/MM/yyyy");
    }

    return title + invoiceNumber + aquisitionDate;
  }
  ...
  renderPurchaseOrderNumber(number) {
    return {
      columns: [
        {
          text: 'Número do Pedido de Locação',
          color: '#aaaaab',
          bold: true,
          fontSize: 10,
          alignment: 'right',
      ...
      content: [
        {
          columns: [
            ...
            [
              {
                text: `Fatura de Locação - ${rental.invoiceNumber}`,
                color: '#333333',
                width: '*',
                fontSize: 28,
                bold: true,
        ...
        '\n\n',
        {
          width: '100%',
          alignment: 'center',
          text: 'Itens do Contrato de Locação',
          bold: true,
          margin: [0, 10, 0, 10],
          fontSize: 15,
        },
      ...
      content: [
        {
          columns: [
            ...
            [
              {
                text: `Fatura de Locação - ${additive.invoiceNumber}`,
                color: '#333333',
                width: '*',
                fontSize: 28,
                bold: true,
